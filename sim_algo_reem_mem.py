...
def procesar(segmentos, reqs, marcos_libres):
    tamaño_pagina = 16  # 32 bytes por página
    ...
    def segmento_valido(dir_virtual):
        for nombre, base, limite in segmentos:
            # Ajuste: incluir el límite superior como válido
    ...
    for req in reqs:
        ...
        if not segmento_valido(req):
            ...
            break  # Continuar con las siguientes solicitudes

        # 2. Calcular página y offset (relativo al segmento base)
        segmento_base = next(base for nombre, base, limite in segmentos if base <= req < base + limite)
        pagina = (req - segmento_base) // tamaño_pagina
        offset = (req - segmento_base) % tamaño_pagina
